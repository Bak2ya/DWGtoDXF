...
처음 실행할 때 브라우저 언어를 자동으로 확인합니다. 화면 상단의 **언어 (Language)**에서 언제든 바꿀 수 있고, 사용자가 직접 선택한 언어는 브라우저에 저장합니다.
...
- 글꼴 변환 3가지
  - 변환 안 함
  - 전체 문자 `wqy-unicode` 적용
  - 한·중·일(CJK) 문자가 포함된 문자만 변경
- UI 언어별 기본 권장값
  - 한국어 / 일본어 → **전체 문자 변경**
  - 영어 / 스페인어 → **변환 안 함**
...
**CJK는 Chinese / Japanese / Korean, 즉 중국어·일본어·한국어 문자권을 뜻합니다.** 이 설명은 웹앱의 글꼴 변환 영역에 항상 표시됩니다.
